...
public class JQAutoCompleteExtended<DATA> extends JQBaseAutoComplete<DATA> {
	...
	private native void pushLOV (String theValue , String theLabel , String theDescription , DATA rawData)/*-{
		...
		this.@id.co.sigma.jquery.client.form.JQBaseAutoComplete::autocompleteArg["source"].push(swap);
	...
	protected native void renderAutoComplete() /*-{
		$wnd["LATEST_AUTO_COMPLETE_RENDER"] = this.@id.co.sigma.jquery.client.form.JQBaseAutoComplete::autocompleteArg;
		try{
			var swapThis = this ; 
			var idPagar = this.@id.co.sigma.jquery.client.form.JQBaseAutoComplete::elementId ;  
			// focus handler
			...
			.data( "autocomplete" )._renderItem = 
				function( ul, item ) {
					...
					var generatedLabel = swapThis.@id.co.sigma.jquery.client.form.JQAutoCompleteExtended::generateLabelBridget(Ljava/lang/Object;)( item.originalData)  ; 
					return $wnd.$( "<li></li>" )
